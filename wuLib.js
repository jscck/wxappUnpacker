...
function mkdirs(dir, cb) {
  ioLimit.runWithCb(fs.stat.bind(fs), dir, (err, stats) => {
    if (err) mkdirs(path.dirname(dir), () => fs.mkdir(dir, cb));
      ...
      throw Error(
        dir + " was created as a file, so we cannot put file into it."
      );
    else cb();
  });
}
...
function commandExecute(cb, helper) {
  ...
  let orders = [];
  for (let order of process.argv)
    if (order.startsWith("-")) orders.push(order.slice(1));
  ...
  function doNext() {
    ...
    if (nxt.done) {
      ...
      called = true;
      if (faster)
        fastCnt.encount(), cb(nxt.value, fastCnt.decount, orders), doNext();
