...
function catchZGroup(code, groupPreStr, cb) {
  ...
  for (let preStr of groupPreStr) {
    ...
    content =
      content.slice(0, content.indexOf("})(__WXML_GLOBAL__.ops_cached.$gwx")) +
      "})(z);";
...
function catchZ(code, cb) {
  ...
  code = code.slice(
    code.indexOf("(function(z){var a=11;function Z(ops){z.push(ops)}"),
    lastPtr + 4
  );
  vm.run(code);
  cb(z);
    ...
    switch (
      ...
      case 2: { //arithmetic operator
        ...
        function getOp(i) {
          ...
          if (
            ops[i] instanceof Object &&
            typeof ops[i][0] == "object" &&
            ops[i][0][0] == 2
          ) {
            //Add brackets if we need
            if (
              getPrior(op[1], ops.length) >
              getPrior(ops[i][0][1], ops[i].length)
            )
              ret = enBrace(ret, "(");
          }
          return ret;
        ...
        switch (ops.length) {
          ...
          default: {
            ...
            if (a.startsWith("[") && a.endsWith("]")) {
              if (a != "[]") {
                ans = enBrace(
                  a.slice(1, -1).trim() + "," + restoreNext(ops[2], true),
                  "["
                );
                //console.log('-',a);
              } else {
      ...
      case 7: { //get value of str
        switch (ops[1][0]) {
          case 11:
            ans = enBrace(
              "__unTestedGetValue:" + enBrace(jsoToWxon(ops), "["),
              "{"
            );
            break;
          case 3:
