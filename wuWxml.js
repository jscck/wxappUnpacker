        ...
        if (f.callee) {
          if (f.callee.type == "Identifier") {
            switch (f.callee.name) {
              case "_r":
                namePool[f.arguments[0].name].v[f.arguments[1].value] =
                  z[f.arguments[2].value];
                break;
              case "_rz":
              ...
              case "_m":
                {
                  if (dec.init.arguments[2].elements.length > 0)
                    throw Error(
                      "Noticable generics content: " +
                        dec.init.arguments[2].toString()
                    );
                  let mv = {};
                  let name = null,
                    base = 0;
                  for (let x of dec.init.arguments[1].elements) {
                    ...
                    if (!v && typeof v != "number") {
                      if (x.type == "UnaryExpression" && x.operator == "-")
                        ...
                        throw Error(
                          "Unknown type of object in _m attrs array: " + x.type
                        );
                    }
                    if (name === null) {
                      name = v;
                  ...
                  for (let x of dec.init.arguments[2].elements) {
                    ...
                    if (!v && typeof v != "number") {
                      if (x.type == "UnaryExpression" && x.operator == "-")
                        ...
                        throw Error(
                          "Unknown type of object in _mz attrs array: " + x.type
                        );
                    }
                    if (name === null) {
                      name = v;
                      ...
                      for (let f of e.declarations) {
                        if (
                          ...
                          if (f.init.left.callee.name == "_1")
                            ...
                            data =
                              z.mul[zMulName][f.init.left.arguments[1].value];
                        }
                      }
        ...
        if (e.test.callee.name.startsWith("_o")) {
          function parse_OFun(e) {
            if (e.test.callee.name == "_o") return z[e.test.arguments[0].value];
              ...
              throw Error(
                "Unknown if statement test callee name:" + e.test.callee.name
              );
          }
...
function elemToString(elem, dep, moreInfo = false) {
  ...
  for (let v in elem.v)
    ret +=
      " " + v + (elem.v[v] !== null ? '="' + wxmlify(elem.v[v]) + '"' : "");
...
function doWxml(state, dir, name, code, z, xPool, rDs, wxsList, moreInfo) {
  ...
  for (let v in rDs) {
    ...
    let code = oriCode
      .slice(tryPtr + 5, oriCode.lastIndexOf("\n}catch("))
      .trim();
  ...
  try {
    ...
    console.log(
      "error on " +
        name +
        "(" +
        (state[0] === null ? "Main" : "Template-" + state[0]) +
        ")\nerr: ",
      e
    );
    if (state[0] === null) wu.save(path.resolve(dir, name + ".ori.js"), code);
      ...
      wu.save(
        path.resolve(dir, name + ".tem-" + state[0] + ".ori.js"),
